...
tokenizer = AutoTokenizer.from_pretrained(
    r'/Users/avinzhang/git/avin-kit/llm_test/test03/model/uer/gpt2-chinese-cluecorpussmall/models--uer--gpt2-chinese-cluecorpussmall/snapshots/c2c0249d8a2731f269414cc3b22dff021f8e07a3')
model = AutoModelForCausalLM.from_pretrained(
    r'/Users/avinzhang/git/avin-kit/llm_test/test03/model/uer/gpt2-chinese-cluecorpussmall/models--uer--gpt2-chinese-cluecorpussmall/snapshots/c2c0249d8a2731f269414cc3b22dff021f8e07a3')

# 加载自己的训练权重
model.load_state_dict(torch.load("param/net.pt", map_location="cpu"))


# 定义函数，用于生成五言绝句 text 是提示词，row是生成文本的行数，col是每行的字符数
def generate(text, row, col):
    ...
    def generate_loop(data):
        ...
        with torch.no_grad():
            out = model(**data)
        # 获取最后一个字 logits 未归化一化的概率输出
        out = out["logits"]
        # 选择每个序列的最后一个logits,对应于下一个词的预测
        out = out[:, -1]

        # 找到概率排前50的值，以此为分界线，小于该值的全部舍去
        topk_value = torch.topk(out, 50).values
        # 获取每个输出序列中前50个最大的logits，未保持原维度不变，需要对接结果增加一个维度，因为索引操作全降维
        topk_value = topk_value[:, -1].unsqueeze(dim=1)
        # 将所有小于第50大的值logits设置为负无穷，减少低概率词的选择
        out = out.masked_fill(out < topk_value, -float("inf"))

        # 将特殊符号的logtis 值设置为负无穷，防止模型生成这些符号
        for i in ",.()[]{}《》【】「」，。":
        ...
        # 更新 token 的ID类型，通常在bertm模型中使用，但是在GPT模型中是不用的
        ...
        if data["input_ids"].shape[1] >= row * col + row + 1:
            ...
            return data
        # 如果长度未达到要求，递归调用 generate_loop 函数继续生成文本
    ...
    # 创建一个与input_ids形状相同的全1张量，用与注意力掩码
    data["attention_mask"] = torch.ones_like(data["input_ids"])
    # 创建一个与input_ids 形状相同的全0张量，用于token类型ID
    data["token_type_ids"] = torch.zeros_like(data["input_ids"])
    # 复制input_ids到labels，用于模型的目标
    data["labels"] = data["input_ids"].clone()

    # 调用 generate_loop 函数开始生成文本
